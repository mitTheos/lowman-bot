...
console.log(best)
        const embed = new EmbedBuilder({
          ...
          "fields": [
          {
            "name": convertTime(best.lw),
            "value": "\u200B"
          }
        ],
          "image": {
          "url": `https://imgur.com/Vs3CemK.png`,
            "height": 0,
            "width": 0
        }
        });
...
const getBest =(data,callback)=> {
  ...
  for (const e of data) {
    getPb(e["d2MembershipId"], (pb) => {
      if (pb.playedUsersCountMonthly > best.playedUsersCountMonthly) {
        best.playedUsersCountMonthly = pb.playedUsersCountMonthly;
        best.playedUsersMonthly = pb.playedUsersMonthly
      }
