    ...
    getInstances(username, (returnList) => {
        if (returnList !== null) {
            ...
            addPlayers(username, instances, hashcodeMap, (list) => {
                callback(new MonthlyStats(list[0], list[1]));
            });
        } else {
            callback(null)
        }
...
class MonthlyStats {
    ...
    constructor(playedWithList, lowmanList, date) {
        this.playerCount = playedWithList.length;
        this.pb = new MonthlyPB(lowmanList);
        this.date = date;
    }
}